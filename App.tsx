...
export default function App() {
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden transition-colors duration-300">
          ...
          <div className="flex items-center w-[250px] flex-shrink-0">
              {(viewMode === 'calendar' || viewMode === 'day') && (
                 <div className="flex items-center animate-in fade-in duration-200">
                   ...
                   <span className="text-lg sm:text-xl font-bold text-gray-900 dark:text-zinc-100 mx-2 min-w-[80px] sm:min-w-[120px] text-center truncate">{viewMode === 'day' ? (getTodayString(currentDate) === TODAY ? '今天' : `${currentDate.getMonth()+1}月${currentDate.getDate()}日`) : `${currentDate.getFullYear()}年 ${currentDate.getMonth() + 1}月`}</span>
      ...
      <CommandPalette isOpen={isCommandPaletteOpen} onClose={() => setIsCommandPaletteOpen(false)} commands={commands} tasks={tasks ?? []} projects={projects ?? []} onTaskSelect={(task) => openEditModal(task)} />
      ...
      <SettingsModal isOpen={isSettingsOpen} onClose={() => setIsSettingsOpen(false)} settings={aiSettings} onSave={setAiSettings} currentTheme={theme} onThemeChange={setTheme} hotkeys={hotkeys} onHotkeysChange={setHotkeys} defaultHotkeys={DEFAULT_HOTKEYS} addToast={addToast} tasks={tasks ?? []} />
      <TaskDetailPanel
        task={selectedTask}
