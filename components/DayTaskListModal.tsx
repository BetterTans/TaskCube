...
/**
 * DayTaskListModalProps 接口定义了每日任务列表模态框的属性。
 * @property {boolean} isOpen - 模态框是否可见。
 * @property {function} onClose - 关闭模态框的回调。
 * @property {string} dateStr - 当前显示的日期字符串 (YYYY-MM-DD)。
 * @property {Task[]} tasks - 该日期的所有任务。
 * @property {function} onTaskClick - 点击任务项的回调，通常用于打开任务详情。
 * @property {function} onCreateTask - 点击“创建新任务”按钮的回调。
 * @property {function} onToggleTask - 切换任务完成状态的回调。
 */
...
/**
 * 每日任务列表模态框组件。
 * 当用户在月视图 (`FullCalendar`) 中点击某个日期数字时，会弹出此模态框，
 * 集中展示该日的所有任务。
 */
