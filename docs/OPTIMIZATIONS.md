...
### 1. 统一的视觉语言 (Unified Visual Language)
*   **状态**: ✅ **已实现 (v3.1.1)**
*   **问题描述**: 应用中不同视图（看板、列表、任务详情）对艾森豪威尔四象限的颜色表示不一致，降低了信息传递的效率。
*   **实现方案**:
    *   设计并实施了一套全局统一的、符合直觉的颜色方案，用于明确表示每个象限的“行动指令”：
        *   🟥 **Q1 (重要 & 紧急):** 炽热红 - "立即处理"
        *   🟩 **Q2 (重要 & 不紧急):** 规划绿 - "计划执行"
        *   🟧 **Q3 (紧急 & 不重要):** 警告橙 - "审慎处理"
        *   🟦 **Q4 (不重要 & 不紧急):** 冷静蓝 - "暂缓排除"
    *   **结果**: 在整个应用中，用户看到的四象限颜色完全一致，极大地增强了视觉识别度和信息传达的一致性。

### 2. 优雅的微交互 (Elegant Micro-interactions)
*   **状态**: ✅ **已实现 (v3.1.1)**
*   **问题描述**: “选择前置任务”等浮窗组件的出现动画是从屏幕左上角“飞入”，效果突兀，不符合 iOS/macOS 的设计习惯。
*   **实现方案**:
    *   重构了浮窗组件的渲染和动画逻辑。现在，组件会先在最终位置以 `opacity: 0` 的状态渲染，然后再触发一个平滑的 `fade-in` 和 `zoom-in` 动画。
    *   同时，为浮窗增加了毛玻璃背景 (backdrop-blur)，提升了视觉层次感和精致度。
    *   **结果**: 动画效果变得优雅、自然，与应用的整体设计风格更加协调，提升了操作的流畅感。

### 3. 加载状态骨架屏 (Loading Skeletons)
*   **现状**: 数据加载时（虽然 Dexie 很快，但初次加载或大量查询时仍有微小延迟），界面可能短暂空白。
*   **优化方案**: 在数据加载期间，显示与最终内容布局相似的骨架屏。这能极大地改善感知性能，让用户感觉应用响应更快，并避免内容加载时产生的界面布局跳动。

### 4. 上下文感知的界面 (Context-Aware UI)
*   **现状**: 应用顶部的操作栏在所有视图下基本是固定的。
*   **优化方案**: 让顶栏的控件根据当前视图动态变化。例如，在“看板视图”中，显示“按时间范围筛选”；在“列表视图”中，则隐藏日期导航，让界面更简洁，只展示与当前上下文最相关的操作。

### 5. 过滤器持久化与预设 (Filter Persistence & Presets)
