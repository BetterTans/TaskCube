# 🚀 NextDo 开发路线图 (Roadmap)

本文档记录了 NextDo 项目的迭代历史、未来架构演进方向与功能开发计划。
...
*   **功能**:
    ...
    *   **桌面应用 (Tauri)**: 从 Web 应用扩展为跨平台桌面应用，使用 Tauri（Rust）提供 macOS、Windows、Linux 安装包，实现本地持久化存储。

### 阶段十三：筛选修复与体验打磨 (v3.2.0)
*   **状态**: ✅ 已上线
*   **核心目标**: 修复筛选器缺陷，重构导航体验，增强数据兼容性。
*   **功能**:
    *   **筛选器全局生效**: 侧边栏项目/象限筛选现在在所有视图（四象限/月/日/列表）中一致生效。
    *   **顶部 Tab 栏**: 视图切换从侧边栏移至 Header 顶部（Notion/Linear 风格 Segmented Control）。
    *   **侧边栏职责纯化**: 移除视图切换，侧边栏专注于项目/象限筛选和管理入口。
    *   **筛选指示条**: 非月视图显示筛选激活状态条，支持一键清除。
    *   **侧边栏滚动**: 项目/象限列表添加滚动容器，防止溢出。
    *   **数据兼容性**: 自动恢复功能新增 v1.0 格式向后兼容。
    *   **代码重构**: `useCalendarFilters` → `useTaskFilters`。
...
#### 2. 专注模式 (Focus Mode)
*   **目标**: 将 NextDo 与“心流”工作状态相结合，帮助用户专注地完成单个任务。
*   **详情**:
...
*   **目标**: 让用户可以根据自己的喜好，打造独一无二的 NextDo。
*   **详情**:
